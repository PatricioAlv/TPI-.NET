...
Write-Host "Iniciando API Gateway (Puerto 8000)..." -ForegroundColor Green
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$projectRoot\src\ApiGateway'; Write-Host 'API GATEWAY - Puerto 8000' -ForegroundColor Yellow; dotnet run"
...
Write-Host "  - API Gateway:      http://localhost:8000 (USAR ESTE CON NGROK)" -ForegroundColor Cyan
Write-Host ""
